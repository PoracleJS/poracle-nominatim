...
RUN if [ "$importable_filename" = "" ]; then importable_filename=geoDataDefault.osm.pbf; fi; if [ "$import_threads" = "" ]; then import_threads=4; fi;  echo "sh /app/init.sh /data/${importable_filename}.osm.pbf postgresdata ${import_threads}" && sh /app/init.sh /data/${importable_filename}.osm.pbf postgresdata ${import_threads};
